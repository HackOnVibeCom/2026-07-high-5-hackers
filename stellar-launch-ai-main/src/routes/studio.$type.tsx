...
import { ArrowLeft, Copy, RefreshCcw, Scissors, Wand2, Check, History } from "lucide-react";
...
function AssetWorkspace() {
  ...
  const [text, setText] = useState(sampleAssetContent[type] ?? "Draft your asset here.");
  ...
  const [history, setHistory] = useState<{ label: string; text: string }[]>([
    ...
    { label: "v2 — warmer tone", text: (sampleAssetContent[type] ?? "").replace(/\n\n/g, "\n\n") },
  ]);
    ...
    setTimeout(() => {
      ...
      const next =
        kind === "shorten"
          ...
          : `${text}\n\n— (${kind}) ${stamp}`;
      setText(next);
      setHistory((h) => [{ label: `v${h.length + 1} — ${kind}`, text: next }, ...h]);
      setLoading(null);
    }, 900);
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link to="/studio" className="inline-flex items-center gap-1 text-sm text-neutral-600 hover:text-neutral-900">
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="rounded-xl border border-neutral-200 bg-white p-5">
          <div className="mb-4 flex flex-wrap items-center gap-2">
            <Toolbar disabled={!!loading} loadingKind={loading} onRegen={() => mutate("regen")} onImprove={() => mutate("improve")} onShorten={() => mutate("shorten")} />
...
function Toolbar({
  ...
  const B = ({
    onClick, icon, label, kind, primary,
  }: {
    onClick: () => void; icon: React.ReactNode; label: string; kind: "regen" | "improve" | "shorten"; primary?: boolean;
  }) => (
    <button
      onClick={onClick}
      disabled={disabled}
  ...
  return (
    <>
      <B onClick={onRegen} icon={<RefreshCcw className="h-4 w-4" />} label="Regenerate" kind="regen" primary />
      <B onClick={onImprove} icon={<Wand2 className="h-4 w-4" />} label="Improve" kind="improve" />
      <B onClick={onShorten} icon={<Scissors className="h-4 w-4" />} label="Shorten" kind="shorten" />
    </>
  );
}
    ...
    return (
      <div className="rounded-xl border border-neutral-200 bg-neutral-50 p-5">
        <div className="flex items-center gap-3">
          <div className="grid h-14 w-14 place-items-center rounded-xl bg-teal-100 text-2xl">🌿</div>
          <div>
            <div className="font-display text-lg font-semibold">Fernly</div>
            <div className="text-xs text-neutral-500">Health & Fitness · 4.8 ★</div>
          </div>
          <button className="ml-auto rounded-full bg-amber-500 px-4 py-1.5 text-xs font-medium text-white">Get</button>
        </div>
    ...
    return (
      <div className="mx-auto max-w-[520px] rounded-xl border border-neutral-200 bg-white p-4">
        <div className="flex gap-3">
          <div className="grid h-10 w-10 place-items-center rounded-full bg-neutral-800 text-sm text-white">AN</div>
          <div className="min-w-0 flex-1">
            <div className="text-sm"><span className="font-medium">Ana Ng</span> <span className="text-neutral-500">@anabuilds · now</span></div>
  ...
  if (type === "linkedin") {
    return (
      <div className="mx-auto max-w-[560px] rounded-xl border border-neutral-200 bg-white p-4">
        <div className="flex items-center gap-3">
          <div className="grid h-11 w-11 place-items-center rounded-full bg-blue-100 text-blue-800">AN</div>
          <div>
